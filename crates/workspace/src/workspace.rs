...
) -> Option<Task<Result<Box<dyn item::FollowableItemHandle>>>>;
type FollowableItemBuilders = HashMap<
    TypeId,
    (
        FollowableItemBuilder,
        fn(AnyViewHandle) -> Box<dyn item::FollowableItemHandle>,
    ),
>;
...
pub fn register_project_item<I: item::ProjectItem>(cx: &mut MutableAppContext) {
...
pub fn register_followable_item<I: item::FollowableItem>(cx: &mut MutableAppContext) {
...
enum FollowerItem {
    ...
    Loaded(Box<dyn item::FollowableItemHandle>),
}
...
impl Workspace {
    ...
    pub fn item_of_type<T: item::Item>(&self, cx: &AppContext) -> Option<ViewHandle<T>> {
        self.items_of_type(cx).max_by_key(|item| item.id())
    }

    pub fn items_of_type<'a, T: item::Item>(
        &'a self,
        cx: &'a AppContext,
