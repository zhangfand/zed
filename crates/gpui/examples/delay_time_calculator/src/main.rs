...
use tap_tempo::TapTempo as TapTempoInternal;
use ui::prelude::*;

use gpui::{
    div, App, Bounds, GlobalPixels, Point, Render, Size, ViewContext, WindowBounds, WindowOptions,
};
...
impl Default for TapTempo {
    fn default() -> Self {
        ...
        Self {
            tap_tempo: TapTempoInternal::new(),
            tempo: Some(tempo),
...
fn main() {
    let app = App::production(Arc::new(()));
        ...
        cx.open_window(
            WindowOptions {
                bounds: WindowBounds::Fixed(Bounds {
                    ...
                    size: Size {
                        width: GlobalPixels::from(500.),
                        height: GlobalPixels::from(500.),
                    },
                }),
                ..WindowOptions::default()
            },
            move |cx| cx.new_view(|_| TapTempo::default()),
        );
    });
}

impl Render for TapTempo {
    fn render(&mut self, _: &mut ViewContext<Self>) -> impl Element {
        v_stack().child(div().child(Button::new("tap", "Tap")))
    }
}
