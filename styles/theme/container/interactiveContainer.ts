...
import { ContainerStyle, DEFAULT_CONTAINER_INTENSITY_SET, containerStyle } from "./containerStyle";
...
function interactiveContainerStyle({ theme, options }: InteractiveContainerProps): Interactive<ContainerStyle> {
  const color = useColors(theme);

  const mergedOptions = options ? deepmerge(DEFAULT_OPTIONS, options) : DEFAULT_OPTIONS;

  const stateIntensities = buildIntensitiesForStates(theme, "interactiveContainer", mergedOptions.intensitySet);
