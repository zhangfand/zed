...
function buttonStyles(
  button: Button,
  state: keyof Interactive<InteractiveState>
) {
  const s = button[state]

  const { background, margin, padding, borderRadius, border } = s.container
  ...
  styles.width =
    typeof s.container.width === "number" ? `${s.container.width}px` : "auto"
  styles.height =
    typeof s.container.height === "number" ? `${s.container.height}px` : "auto"
