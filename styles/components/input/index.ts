...
import { ContainerOptions, ContainerStyle, Interactive, buildIntensitiesForStates, containerStyle } from "@theme/container"
import { IntensitySet, intensity, resolveElementIntensities } from "@theme/intensity"
...
//* A single line input */
export function inputStyle({ theme, options }: InputProps): Interactive<InputStyle> {
  ...
  const states = buildIntensitiesForStates(theme, "input", resolvedIntensities)
  ...
  const buildStates = (intensities: IntensitySet): InputStyle => {
    const updatedContainer = {
      ...
      border: border(theme, intensities.border, { color: mergedOptions.color }),
    }
